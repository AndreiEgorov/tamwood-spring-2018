 ...
 */
  ...
  */
 function tamwood_comment_count() {
  ...
  */
 function tamwood_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && tamwood_categorized_blog() ) {
  ...
  * @return bool
  */
 function tamwood_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'tamwood_categories' ) ) ) {
 		...
 		set_transient( 'tamwood_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so tamwood_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so tamwood_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in tamwood_categorized_blog.
  */
 function tamwood_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'tamwood_categories' );
 }
 add_action( 'edit_category', 'tamwood_category_transient_flusher' );
 add_action( 'save_post',     'tamwood_category_transient_flusher' );
  ...
  */
 function tamwood_comment_list( $comment, $args, $depth ) {
  ...
  */
 function tamwood_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
